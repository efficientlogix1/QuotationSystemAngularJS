app.controller("BuyerController", function ($scope, $compile) {
    ...
    $scope.BuyerFetchById = function (recordId) {
        RedirectDelay('/OrderRequest/RequesterRequestSetup?requestID=' + recordId + '&isEdit=true');
    ...
    function BuyerBindGrid() {
        ...
        $("#BuyerTable").advancedDataTable({
            url: "/OrderRequest/FetchRequesterOrders",
            ...
            bindingData: [
                ...
                },
                {
                    "render": function (data, type, row) {
                        ...
                        //});
                        if (row.ActionName == "Incomplete")
                        {
                            return '<a class="btn btn-info" href="javascript: void(0);" onclick="BuyerFetchById(' + row.Id + ')">Edit</a>';
                        }
                        else {
                                
                        return '<a class="btn btn-info" href="javascript: void(0);" onclick="BuyerFetchById(' + row.Id + ')">View</a>';
                        }
                    },
                    //  "className": "dropdown",
                    "orderable": false
