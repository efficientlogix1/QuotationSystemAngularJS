app.controller("RequestController", function ($scope, $http, $compile) {
    ...
    PreBind();
    function PreBind() {
        Get('/OrderRequest/PrebindRequesterOrderSetup', true).then(function (d) {
            if (d.msg.Success) {
            ...
            Get('/OrderRequest/FetchRequestByID?requestID=' + editRequests.requestID, true).then(function (d) {
                if (d.msg.Success) {
                    ...
                    for (var i = 0; i < $scope.request.BuyerRequests.length; i++) {
                        var option = 'number:' + $scope.request.BuyerRequests[i].BuyerID;
                        lstBuyers.push(option);
                    }
                    $('#SavedBuyers').val(lstBuyers).trigger('change');
                    ...
                    if (d.Data.ActionID == 4) {
                        $scope.request.isEdit = true;
                        $('#Buyers').val(lstBuyers).trigger('change');
                    }
                    else {
                        $scope.request.isEdit = false;
                    }
                    $scope.$apply();
    ...
    $scope.SubmitForm = function () {
        ...
        if (flag) {
            if ($("#Buyers").length) {
                $scope.lstSelectedBuyers = $("#Buyers").val();
                if ($scope.lstSelectedBuyers.length != $scope.request.BuyerRequests.length) {
                    for (var i = 0; i < $scope.lstSelectedBuyers.length; i++) {
                        var innerflag = true;
                        var buyerRequest = [];
                        for (var j = 0; j < $scope.request.BuyerRequests.length; j++) {

                            if ($scope.request.BuyerRequests[j].BuyerID == $scope.lstSelectedBuyers[i].split(':')[1]) {
                                innerflag = false;
                                break;
                        ...
                        if (innerflag) {
                            ...
                            $scope.request.BuyerRequests.push({ BuyerID: $scope.lstSelectedBuyers[i].split(':')[1] });
                        }
                    }
                }
            ...
            Post("/OrderRequest/SaveRequesterOrder", { orderRequest: $scope.request, statusID }).then(function (d) {
                if (d.msg.Success) {
